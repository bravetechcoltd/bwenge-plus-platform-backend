...
import { BulkImportJob, BulkImportJobStatus } from "../database/models/BulkImportJob";
...
static async createInstitution(req: Request, res: Response) {
  try {
    ...
    // Parse max_instructors and max_members with defaults
    const parsedMaxInstructors = max_instructors ? parseInt(max_instructors, 10) : 50;
    const parsedMaxMembers = max_members ? parseInt(max_members, 10) : 500;
    ...
    const institution = institutionRepo.create({
      ...
      is_active: true,
      max_instructors: parsedMaxInstructors,
    ...
    if (adminUser) {
      ...
      adminUser.institution_role = InstitutionRole.ADMIN;
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <p><strong>Institution:</strong> ${institution.name}</p>
              <p><strong>Your Role:</strong> Institution Administrator</p>
              <p><strong>Instructor Limit:</strong> ${parsedMaxInstructors}</p>
              <p><strong>Member Limit:</strong> ${parsedMaxMembers}</p>
            </div>
    ...
    // Notify system admins about new institution registration
    NotificationService.onNewInstitutionRegistration(
      institution.name,
      institution.id,
      adminUser.id
    ).catch(() => {});

    // Notify system admins about new institution admin
    const adminName = `${adminUser.first_name || ''} ${adminUser.last_name || ''}`.trim() || adminUser.email;
    NotificationService.onNewInstitutionAdmin(
      adminName,
      adminUser.id
    ).catch(() => {});
...
// Replace the updateInstitution method
static async updateInstitution(req: Request, res: Response) {
  try {
    ...
    // Handle max_instructors and max_members updates
    if (updates.max_instructors !== undefined) {
      const newMaxInstructors = parseInt(updates.max_instructors, 10);
      if (!isNaN(newMaxInstructors) && newMaxInstructors >= 0) {
        // Check if new limit is less than current instructor count
        const memberRepo = dbConnection.getRepository(InstitutionMember);
        const currentInstructorCount = await memberRepo.count({
          where: {
            institution_id: id,
        ...
        if (newMaxInstructors < currentInstructorCount) {
          return res.status(400).json({
            success: false,
            message: `Cannot set instructor limit to ${newMaxInstructors} because there are ${currentInstructorCount} instructors currently. Please remove some instructors first.`,
          });
        }
        institution.max_instructors = newMaxInstructors;
      }
    }

    if (updates.max_members !== undefined) {
      const newMaxMembers = parseInt(updates.max_members, 10);
      if (!isNaN(newMaxMembers) && newMaxMembers >= 0) {
        // Check if new limit is less than current non-instructor member count
        const memberRepo = dbConnection.getRepository(InstitutionMember);
        const currentNonInstructorCount = await memberRepo.count({
          where: {
            institution_id: id,
            role: Not(InstitutionMemberRole.INSTRUCTOR),
            is_active: true,
          },
        });

        if (newMaxMembers < currentNonInstructorCount) {
          return res.status(400).json({
            success: false,
            message: `Cannot set member limit to ${newMaxMembers} because there are ${currentNonInstructorCount} non-instructor members currently. Please remove some members first.`,
          });
        }
        institution.max_members = newMaxMembers;
      }
    }

    // Apply other updates
    if (updates.name) institution.name = updates.name;
    if (updates.type) institution.type = updates.type;
    if (updates.description !== undefined) institution.description = updates.description;
    if (updates.logo_url) institution.logo_url = updates.logo_url;
    if (updates.settings) institution.settings = { ...institution.settings, ...updates.settings };
...
static async addMemberToInstitution(req: Request, res: Response) {
  try {
    ...
    // Check if adding an instructor would exceed the instructor limit
    if (role === InstitutionMemberRole.INSTRUCTOR && currentInstructorCount >= institution.max_instructors) {
      return res.status(400).json({
        success: false,
        message: `Cannot add new instructor. Maximum instructor limit (${institution.max_instructors}) has been reached.`,
        data: {
          limit_reached: true,
          limit_type: "instructors",
          current: currentInstructorCount,
          max: institution.max_instructors,
        },
      });
    }

    // Check if adding a non-instructor member would exceed the member limit
    if (role !== InstitutionMemberRole.INSTRUCTOR && currentNonInstructorCount >= institution.max_members) {
      return res.status(400).json({
        success: false,
        message: `Cannot add new member. Maximum member limit (${institution.max_members}) has been reached.`,
        data: {
          limit_reached: true,
          limit_type: "members",
          current: currentNonInstructorCount,
          max: institution.max_members,
        },
      });
    }
    ...
    if (!user) {
      ...
      if (existingMember) {
        if (existingMember.is_active) {
          ...
          // Reactivate existing membership - still need to check limits
          // But since they were previously a member, we allow reactivation within limits
    ...
    // Send in-app notification to institution admins about new member
    const memberName = `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.email;
    if (role === InstitutionMemberRole.INSTRUCTOR) {
      NotificationService.onNewInstructorJoined(id, memberName, user.id).catch(() => {});
    } else {
      NotificationService.onNewStudentJoined(id, memberName, user.id).catch(() => {});
    }
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        institution: {
          ...
          name: institution.name,
          type: institution.type,
          max_instructors: institution.max_instructors,
          max_members: institution.max_members,
          total_capacity: institution.max_instructors + institution.max_members,
          current_members: role !== InstitutionMemberRole.INSTRUCTOR ? currentNonInstructorCount + 1 : currentNonInstructorCount,
          current_instructors: role === InstitutionMemberRole.INSTRUCTOR ? currentInstructorCount + 1 : currentInstructorCount,
        },
        notification: {
...
// Add a new method to get institution limits
static async getInstitutionLimits(req: Request, res: Response) {
  try {
    const { id } = req.params;
    ...
    const totalMembers = currentInstructorCount + currentNonInstructorCount;
    const totalCapacity = institution.max_instructors + institution.max_members;
    ...
    res.json({
      ...
      data: {
        ...
        current_instructors: currentInstructorCount,
        current_members: currentNonInstructorCount,
        total_current: totalMembers,
        instructors_remaining: Math.max(0, institution.max_instructors - currentInstructorCount),
        members_remaining: Math.max(0, institution.max_members - currentNonInstructorCount),
        can_add_instructor: currentInstructorCount < institution.max_instructors,
        can_add_member: currentNonInstructorCount < institution.max_members,
      },
    });
  } catch (error: any) {
    res.status(500).json({
...
static async getInstitutionById(req: Request, res: Response) {
  ...
  static async getInstitutionMembers(req: Request, res: Response) {
    try {
      ...
      // Filter by active status — default to only active members
      if (is_active !== undefined) {
        queryBuilder.andWhere("member.is_active = :is_active", {
          is_active: is_active === "true"
        });
      } else {
        queryBuilder.andWhere("member.is_active = :is_active", { is_active: true });
      }
      ...
      const enrollmentStats = userIds.length > 0 ? await dbConnection
        .getRepository(Enrollment)
        .createQueryBuilder("enrollment")
        .select("enrollment.user_id", "user_id")
        .addSelect(
          "COUNT(DISTINCT enrollment.course_id) FILTER (WHERE enrollment.status IN (:...enrolledStatuses))",
          "enrolled_courses_count"
        )
        .addSelect(
          "COUNT(DISTINCT enrollment.course_id) FILTER (WHERE enrollment.status = :completedStatus)",
          "completed_courses_count"
        )
        .addSelect("COALESCE(SUM(enrollment.total_time_spent_minutes), 0)", "total_learning_minutes")
        .where("enrollment.user_id IN (:...userIds)", { userIds })
        .setParameter("enrolledStatuses", ["ACTIVE", "COMPLETED"])
        .setParameter("completedStatus", "COMPLETED")
        .groupBy("enrollment.user_id")
        .getRawMany() : [];

      const certificateStats = userIds.length > 0 ? await dbConnection
        .getRepository(Certificate)
        .createQueryBuilder("certificate")
        .select("certificate.user_id", "user_id")
        .addSelect("COUNT(*)", "certificates_earned")
        .where("certificate.user_id IN (:...userIds)", { userIds })
        .groupBy("certificate.user_id")
        .getRawMany() : [];

      const enrollmentStatsMap = enrollmentStats.reduce((map, row) => {
        map[row.user_id] = {
          enrolled_courses_count: Number(row.enrolled_courses_count) || 0,
          completed_courses_count: Number(row.completed_courses_count) || 0,
          total_learning_minutes: Number(row.total_learning_minutes) || 0,
        };
        return map;
      }, {} as Record<string, { enrolled_courses_count: number; completed_courses_count: number; total_learning_minutes: number; }>);

      const certificateStatsMap = certificateStats.reduce((map, row) => {
        map[row.user_id] = Number(row.certificates_earned) || 0;
        return map;
      }, {} as Record<string, number>);

      const instructorUserIds = members
        .filter((member) => member.role === InstitutionMemberRole.INSTRUCTOR)
        .map((member) => member.user.id);

      const coursesByInstructorId: Record<string, Array<any>> = {};

      if (instructorUserIds.length > 0) {
        const primaryCourses = await dbConnection
          .getRepository(Course)
          .createQueryBuilder("course")
          .where("course.instructor_id IN (:...userIds)", { userIds: instructorUserIds })
          .getMany();

        const assignedCourses = await dbConnection
          .getRepository(CourseInstructor)
          .createQueryBuilder("ci")
          .leftJoinAndSelect("ci.course", "course")
          .where("ci.instructor_id IN (:...userIds)", { userIds: instructorUserIds })
          .getMany();

        primaryCourses.forEach((course) => {
          if (!course.instructor_id) return;
          coursesByInstructorId[course.instructor_id] = coursesByInstructorId[course.instructor_id] || [];
          coursesByInstructorId[course.instructor_id].push({
            id: course.id,
            title: course.title,
            thumbnail_url: course.thumbnail_url,
            status: course.status,
        ...
        assignedCourses.forEach((assignment) => {
          ...
          coursesByInstructorId[instructorId] = coursesByInstructorId[instructorId] || [];

          const existing = coursesByInstructorId[instructorId].find((c) => c.id === course.id);
          if (existing) {
            existing.is_primary_instructor = existing.is_primary_instructor || assignment.is_primary_instructor;
            existing.source = existing.source === "primary" ? "primary" : "assigned";
            return;
          }

          coursesByInstructorId[instructorId].push({
            id: course.id,
            title: course.title,
            thumbnail_url: course.thumbnail_url,
            status: course.status,
            ...
            completion_rate: course.completion_rate,
            average_rating: Number(course.average_rating || 0),
            is_primary_instructor: assignment.is_primary_instructor,
            source: "assigned",
          });
        });
      }
      ...
      const sanitizedMembers = members.map((member) => {
        const stats = enrollmentStatsMap[member.user.id] || {
          enrolled_courses_count: 0,
          completed_courses_count: 0,
          total_learning_minutes: 0,
        };

        const instructorCourses = member.role === InstitutionMemberRole.INSTRUCTOR
          ? coursesByInstructorId[member.user.id] || []
          : [];
        ...
        return {
          ...
          user: {
            ...
            country: member.user.country,
            city: member.user.city,
            enrolled_courses_count: stats.enrolled_courses_count,
            completed_courses_count: stats.completed_courses_count,
            total_learning_hours: Number((stats.total_learning_minutes / 60).toFixed(2)),
            certificates_earned: certificateStatsMap[member.user.id] || 0,
...
private static async sendInvitationEmail(
  ...
  static async removeMemberFromInstitution(req: Request, res: Response) {
    try {
      ...
      const user = await userRepo.findOne({
        where: { id: userId },
        relations: ["institution_memberships"],
      });

      if (user) {
        // Get remaining active memberships (excluding the one just removed)
        const remainingMemberships = (user.institution_memberships || []).filter(
          (m) => m.institution_id !== id && m.is_active
        );

        const remainingIds = remainingMemberships.map((m) => m.institution_id);
        const hasRemainingMemberships = remainingIds.length > 0;

        // Determine new primary institution and role
        let newPrimaryId: string | null = null;
        let newInstitutionRole: InstitutionRole | null = null;
        let newBwengeRole: BwengeRole = BwengeRole.LEARNER;

        if (hasRemainingMemberships) {
          // Use first remaining membership as primary
          const newPrimary = remainingMemberships[0];
          newPrimaryId = newPrimary.institution_id;
          ...
          const mapped = roleMap[newPrimary.role as InstitutionMemberRole] || roleMap[InstitutionMemberRole.MEMBER];
          newInstitutionRole = mapped.inst;
          newBwengeRole = mapped.bwenge;
        }
        ...
        await userRepo
          ...
          .update(User)
          .set({
            is_institution_member: hasRemainingMemberships,
            institution_ids: hasRemainingMemberships ? remainingIds : () => "NULL",
            primary_institution_id: newPrimaryId as any,
            institution_role: newInstitutionRole as any,
            bwenge_role: hasRemainingMemberships ? newBwengeRole : BwengeRole.LEARNER,
          })
  ...
  static async verifyInviteToken(req: Request, res: Response) {
    try {
      ...
      return res.json({
        ...
        data: {
          institution: {
            ...
            logo_url: invitation.institution.logo_url,
            description: invitation.institution.description,
            type: invitation.institution.type,
            slug: invitation.institution.slug,
          },
  ...
  static async acceptInvite(req: Request, res: Response) {
    try {
      ...
      if (existing && existing.is_active) {
        return res.status(409).json({ success: false, message: "You are already a member of this institution" });
      }

      // Check member/instructor limits before joining
      const institutionRepo = dbConnection.getRepository(Institution);
      ...
      // Add or reactivate institution member
      let member: InstitutionMember;
      if (existing && !existing.is_active) {
        // Reactivate previously deactivated membership
      ...
      // Update user's institution fields via query builder for reliable persistence
      const userRepo = dbConnection.getRepository(User);
      ...
      if (user) {
        ...
        // For reusable invite links, preserve the joined user's email on the invitation record
        if (invitation.type === InvitationType.LINK && !invitation.email) {
          invitation.email = user.email;
          await invitationRepo.save(invitation);
        }
      }

      // Mark email-type invitations as accepted; link-type stays active for reuse
      if (invitation.type === InvitationType.EMAIL) {
        invitation.status = InvitationStatus.ACCEPTED;
        await invitationRepo.save(invitation);
      }

      // Re-fetch the user so the response has fresh institution fields
      const freshUser = await dbConnection.getRepository(User).findOne({ where: { id: userId } });

      return res.json({
        success: true,
        message: `Successfully joined ${invitation.institution.name}!`,
        data: {
          institution_id: institutionId,
          institution_name: invitation.institution.name,
          role,
          member_id: member.id,
          // Expose updated fields so frontend can sync without an extra /profile call
          user: freshUser ? {
            institution_ids: freshUser.institution_ids,
            primary_institution_id: freshUser.primary_institution_id,
            institution_role: freshUser.institution_role,
            is_institution_member: freshUser.is_institution_member,
  ...
  static async inviteUser(req: Request, res: Response) {
    try {
      ...
      const invitedBy = (req as any).user?.id ?? null;
  ...
  static async generateInviteLink(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      // Check member/instructor limits before generating link
      const memberRepo = dbConnection.getRepository(InstitutionMember);
      const currentInstructorCount = await memberRepo.count({ where: { institution_id: id, role: InstitutionMemberRole.INSTRUCTOR, is_active: true } });
      const currentNonInstructorCount = await memberRepo.count({ where: { institution_id: id, role: Not(InstitutionMemberRole.INSTRUCTOR), is_active: true } });

      const normalizedRole = role.toUpperCase();
      if (normalizedRole === "INSTRUCTOR" && currentInstructorCount >= institution.max_instructors) {
        return res.status(400).json({
          success: false,
          message: `Cannot generate instructor invite link. Maximum instructor limit (${institution.max_instructors}) has been reached.`,
          data: { limit_reached: true, limit_type: "instructors", current: currentInstructorCount, max: institution.max_instructors },
        });
      }
      if (normalizedRole !== "INSTRUCTOR" && currentNonInstructorCount >= institution.max_members) {
        return res.status(400).json({
          success: false,
          message: `Cannot generate invite link. Maximum member limit (${institution.max_members}) has been reached.`,
          data: { limit_reached: true, limit_type: "members", current: currentNonInstructorCount, max: institution.max_members },
        });
      }
      ...
      const link = `${process.env.CLIENT_URL}/join?token=${token}`;
  ...
  static async resendInvitation(req: Request, res: Response) {
    try {
      ...
      const invitationRepo = dbConnection.getRepository(InstitutionInvitation);
      const invitation = await invitationRepo.findOne({
        where: { id: inviteId, institution_id: id },
      });
      ...
      if (!invitation.email) {
        return res.status(400).json({ success: false, message: "No email associated with this invitation" });
      }
      ...
      await sendEmail({
        to: invitation.email,
        subject: `Reminder: Invitation to join ${institution?.name || "our institution"}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2>You still have a pending invitation!</h2>
            <p>You were invited to join <strong>${institution?.name || "our institution"}</strong> as a ${invitation.role || "member"}.</p>
            ${invitation.message ? `<p><em>${invitation.message}</em></p>` : ""}
            <p>Click the link below to accept:</p>
            <a href="${process.env.CLIENT_URL}/register?institution=${id}&email=${invitation.email}"
               style="display: inline-block; padding: 12px 24px; background-color: #5B7FA2; color: white; text-decoration: none; border-radius: 5px; margin: 20px 0;">
              Accept Invitation
            </a>
          </div>
  ...
  static async bulkInvite(req: Request, res: Response) {
    try {
      ...
      const invitationRepo = dbConnection.getRepository(InstitutionInvitation);
      const results = { succeeded: 0, failed: 0, invitations: [] as any[] };
      ...
      for (const email of emails) {
        try {
          ...
          results.invitations.push(saved);
          results.succeeded++;
        } catch {
          results.failed++;
        }
  ...
  static async getInvitations(req: Request, res: Response) {
    try {
      ...
      const query = invitationRepo
        ...
        .where("inv.institution_id = :id", { id });
      ...
      query.orderBy("inv.created_at", "DESC").skip((pageNum - 1) * limitNum).take(limitNum);
  ...
  static async bulkImport(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const file = req.file;
      const defaultRole = (req.body.default_role || "MEMBER").toUpperCase();
      ...
      // Create job record
      const jobRepo = dbConnection.getRepository(BulkImportJob);
      ...
      res.json({
        ...
        data: { job_id: job.id, status: job.status, total: job.total },
      });
      ...
      (async () => {
        ...
        for (let i = 0; i < dataRows.length; i++) {
          try {
            ...
            const role = (row.role || defaultRole).toUpperCase();

            if (!email || !first_name) {
              errors.push(`Row ${i + 2}: Missing email or first name`);
              failed++;
              continue;
            }
          ...
          if ((i + 1) % 10 === 0) {
            await jobRepo.update(job.id, { processed: i + 1, succeeded, failed, errors });
          }
        }
  ...
  static async getBulkImportJobs(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      const [jobs, total] = await jobRepo.findAndCount({
  ...
  // ==================== GET BULK IMPORT JOB ====================
  static async getBulkImportJob(req: Request, res: Response) {
    try {
      const { id, jobId } = req.params;

      const jobRepo = dbConnection.getRepository(BulkImportJob);
      const job = await jobRepo.findOne({ where: { id: jobId, institution_id: id } });
