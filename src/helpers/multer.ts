...
export const uploadAssessmentFileFrontend = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedTypes = [
      ...
      "text/x-php", "text/html", "text/css", "application/javascript", "application/typescript",
    ...
    if (allowedTypes.includes(file.mimetype) || allowedExtensionsList.includes(ext)) {
      return cb(null, true)
    }
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    else if (file.mimetype.startsWith("video/")) folder = "uploads/video"
    else if (file.mimetype.startsWith("text/") || file.mimetype.includes("document") || file.mimetype.includes("pdf")) folder = "uploads/documents"
    else if (file.mimetype.includes("font")) folder = "uploads/fonts"
    else if (file.mimetype.includes("zip") || file.mimetype.includes("compressed")) folder = "uploads/archives"
    else if (file.mimetype.includes("application/x-msdownload") || file.mimetype.includes("application/x-executable")) folder = "uploads/executables"
    else if (file.mimetype.includes("application/javascript") || file.mimetype.includes("text/x-")) folder = "uploads/code"
...
const fileFilter = (req: Request, file: File, cb: multer.FileFilterCallback) => {
  const ext = path.extname(file.originalname).toLowerCase()
  if (allAllowedExtensions.includes(ext)) return cb(null, true)
...
const upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 10 * 1024 * 1024, files: 15, fieldSize: 10 * 1024 * 1024 },
})

// ==================== LARGE UPLOAD INSTANCE (videos up to 4GB) ====================
const uploadLarge = multer({
  storage: largeFileStorage,
  fileFilter,
  limits: { fileSize: 4 * 1024 * 1024 * 1024, files: 20, fieldSize: 10 * 1024 * 1024 },
})
  ...
  if (error instanceof multer.MulterError) {
    switch (error.code) {
      case "LIMIT_FILE_SIZE":
        return res.status(400).json({ success: false, message: "File too large. Videos up to 4GB and other files up to 100MB are allowed.", error: error.message, details: `File '${error.field}' exceeded size limit` })
      case "LIMIT_FILE_COUNT":
        return res.status(400).json({ success: false, message: "Too many files. Maximum 20 files allowed.", error: error.message })
      case "LIMIT_UNEXPECTED_FILE":
        return res.status(400).json({ success: false, message: "Unexpected file field.", error: error.message, details: `Field '${error.field}' was not expected` })
      case "LIMIT_FIELD_KEY":
        return res.status(400).json({ success: false, message: "Field name too long.", error: error.message })
      case "LIMIT_FIELD_VALUE":
        return res.status(400).json({ success: false, message: "Field value too large.", error: error.message })
      case "LIMIT_FIELD_COUNT":
        return res.status(400).json({ success: false, message: "Too many fields.", error: error.message })
      case "LIMIT_PART_COUNT":
        return res.status(400).json({ success: false, message: "Too many parts.", error: error.message })
      default:
        return res.status(400).json({ success: false, message: "File upload error.", error: error.message, details: `Multer error code: ${error.code}` })
    }
  }
  if (error.message && error.message.includes("Invalid file type")) {
    return res.status(400).json({ success: false, message: error.message, error: "Invalid file type", details: `Allowed types: ${Object.keys(allowedExtensions).join(", ")}`, allowedExtensions })
  }
  next(error)
}
...
export const uploadContributionFiles = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    else cb(new Error("Invalid file type. Only PDF, DOC, DOCX, TXT, and images are allowed."))
  }
}).fields([{ name: "contribution_files", maxCount: 10 }])

export const uploadSingle = upload.single("document")
export const uploadMultiple = upload.array("documents", 15)
export const uploadPropertyImages = upload.array("images", 10)

// ==================== GENERATE DYNAMIC LESSON FIELDS ====================
// Supports up to 20 modules × 30 lessons = 600 potential field combinations.
// NEW: now also generates .thumbnail and .materials[N] field patterns.
const generateLessonFields = () => {
  const fields: { name: string; maxCount: number }[] = []
  for (let modIdx = 0; modIdx < 20; modIdx++) {
    for (let lesIdx = 0; lesIdx < 30; lesIdx++) {
      // Video (primary pattern used by controller)
      fields.push({ name: `modules[${modIdx}].lessons[${lesIdx}].video`, maxCount: 1 })
      // Video (alternate bracket pattern)
      fields.push({ name: `modules[${modIdx}][lessons][${lesIdx}][video]`, maxCount: 1 })

      // Lesson thumbnail image (NEW)
      fields.push({ name: `modules[${modIdx}].lessons[${lesIdx}].thumbnail`, maxCount: 1 })

      // Lesson materials — up to 10 files per lesson (NEW)
      for (let matIdx = 0; matIdx < 10; matIdx++) {
        fields.push({ name: `modules[${modIdx}].lessons[${lesIdx}].materials[${matIdx}]`, maxCount: 1 })
      }

      // Legacy resource upload fields
...
export const uploadFields = uploadLarge.fields([
  ...
  // ==================== ALL LESSON MEDIA FIELDS ====================
  ...generateLessonFields(),
])
